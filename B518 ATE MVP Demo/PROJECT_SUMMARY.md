...
- 由於 macOS Tk 的持續置頂會使設定視窗與按鈕反應不穩定，Log Solution 不再呼叫 `-topmost` 或自動搶焦點。需要回到結果看板時，由人員點擊 macOS Dock 的 Log Solution 圖示帶回前景。
...
- 測試時間超限時，各超時 Slot 個別顯示 `TIMEOUT` 並繼續監控其他 Slot；已完成結果保留。只有整輪等待開始逾時才停止全部監控。Session 記錄逾時類型、觸發 Slot、經過時間與設定門檻。
...
## 2026-09-10 BT CaseInfo 條碼欄位語意修正
