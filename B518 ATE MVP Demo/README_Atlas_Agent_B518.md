...
自動同步時，Agent 會逐一讀取並切換七個 slot，再以第二張截圖驗證；
...
active 資料夾中的 `records.csv` 一旦寫出 `MLB_SN` 或 `PrimaryIdentity`，Agent 會優先用它顯示完整條碼；
只含數字的時間、slot 或暫存值不會當作 SN。無 SN FCT Demo 採三層等待：60 秒尚未偵測 active／新結果時
...
PyInstaller 可能顯示找不到 `Cfgmgr32`、`Advapi32`、`setupapi`，這些是 PySerial 掃描到的 Windows
專用 DLL；IOKit／CoreFoundation 則是 macOS 系統 Framework，均不需要複製進 App。這些 WARNING
本身不代表建置失敗。只有腳本最後完成 plist、簽章、相容性驗證、ZIP 與 SHA-256，並顯示
`Candidate built`／`Deliver this ZIP` 時，該產物才可進入實機驗收；單獨出現在 `dist` 的中間 App
不可直接交付。
...
七槽 Profile 的 checkbox 不會在整張畫面尋找：Agent 先定位下方 group0 文字，用它區分下方 slot 區；
接著定位下方七個共用 `slot` 標籤，只在每一個標籤右側找 checked／unchecked 狀態。
這可避開上方結果表格的重複文字，也只需要一組狀態模板。模板名稱與裁切規則請參閱
`templates/README.md`。slot checkbox 狀態採灰階邊緣與勾形幾何比對，因此 HMI 取得焦點後由灰底黑勾
變成藍底或綠底白勾仍可辨識；checked／unchecked 模板仍應框選相同主體範圍。Agent 不會辨識或點擊 group0，
全程只以 slot1～7 的實際狀態為準。若任一 slot 的兩種狀態分數太接近，Agent 會停止並在 Log 列出分數，不會繼續輸入條碼。
每個 slot 的搜尋寬度由同一排相鄰 slot 文字間距計算，會涵蓋卡片最右側的 checkbox，但不跨入
下一張卡片；因此緊湊版、Retina 截圖或整個瀏覽器平移時不需重新調整固定像素寬度。

七槽自動同步時，第一張截圖先確認七個 slot 現況，Agent 僅逐一點擊與本次 JOB 不符的 slot，
再以第二張截圖確認最終狀態。複驗成功後會用最後一張截圖重新定位 SN 輸入框與 OK，
...
切換。此功能不會自動刪除模板製作用截圖。
為避免 checkbox 在測試 HMI 聚焦與未聚焦時呈現不同顏色，按下「擷取螢幕截圖」後會先顯示
Focused 模板提醒。按下確定後 Atlas Agent 會隱藏並保留 5 秒人工操作時間；操作人員必須在
倒數期間**手動點擊測試 HMI 的標題列或空白安全區**，使測試視窗取得焦點。請勿點擊 checkbox，
以免改變 Slot 狀態。倒數結束後 Arduino 才會自動截圖。checked 與 unchecked 模板應從相同
聚焦狀態的截圖製作，並框選相同範圍。
模板視窗請使用內建的「縮小預覽／放大預覽」按鈕；預覽尺寸受到上限保護，且不使用
macOS 視窗的綠色放大按鈕。來源截圖上限為 2400 萬像素，以避免 Tk／AppKit 建立過大的
影像表面而造成記憶體問題。放大預覽會依目前螢幕高度自動限制畫布，固定預留底部的
「取消／儲存模板」操作區，因此在 1440×900 或 1280×1024 等螢幕不需先縮小預覽才能儲存。
...
輸入框與按鈕收到 click 事件。第一次取得焦點時會先以 `M_ABS` 精準定位，再讓標準滑鼠
做一單位往返後點擊；不會把螢幕座標直接當成相對距離，避免 macOS 滑鼠加速度造成衝過頭。
雙螢幕環境重新驗證七槽 checkbox 時，Agent 只會將找到 `dfu7_window` 的截圖交給 checkbox
判讀；另一個螢幕找不到模板屬正常分流。若 checkbox 外觀仍在切換，Agent 會自動再截圖一次。
