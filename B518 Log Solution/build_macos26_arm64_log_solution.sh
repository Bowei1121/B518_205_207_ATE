...
[[ "$(sw_vers -productVersion)" == 26.* ]] || { print -u2 'Requires macOS 26.x.'; exit 1; }
...
DIST=dist-macos26-arm64
BUILD=build-macos26-arm64
rm -rf "$DIST" "$BUILD"
...
for pair in "CFBundleShortVersionString $VERSION" "CFBundleVersion $VERSION" "LSMinimumSystemVersion 26.0"; do
  key=${pair%% *}
  value=${pair#* }
...
ZIP="$DIST/B518-Log-Solution-V${VERSION}-macOS26-arm64.zip"
